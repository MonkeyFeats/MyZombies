	...
	if (carryBlob !is null)
	{
		...
		if (bc.cursorClose)
		{
			if (snap) // if snaps to grid make cursor
			{
				...
				if (carryBlob.getName() == "triangle")
				{
					f32 angle = carryBlob.getAngleDegrees();
					if (angle == 0)
				    {						
						if( map.rayCastSolidNoBlobs(bottomPos, bottomPos+Vec2f(-8,0)) || map.rayCastSolid(bottomPos, bottomPos+Vec2f(0,8))  )
						{
							trisupport = true;
						}
						else
				...
				{					
					trisupport = true; // because no triangle..
				}
				...
				bc.supported = (carryBlob.getShape().getConsts().support > 0 ? map.hasSupportAtPos(bc.tileAimPos) : true) && trisupport;
