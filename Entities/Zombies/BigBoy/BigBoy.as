...
void onInit(CBlob@ this)
{
	...
	this.set_f32(target_searchrad_property, 3600.0f);
	this.set_f32(terr_rad_property, 185.0f);
	this.set_u8(target_lose_random,10);
...
{
    return false; // this.getHealth()<0.0 || this.getTeamNum() == byBlob.getTeamNum(); //maybe make a knocked out state? for loading to cata?
}
		...
		if (lastbite > bitefreq)
		{
			...
			if(this.get_u8(state_property) == MODE_TARGET )
			{
				...
				else vel = Vec2f(1,0);
				{
					...
					if (map.getHitInfosFromArc( this.getPosition()- Vec2f(2,-4).RotateBy(-vel.Angle()), -vel.Angle(), 90, this.getRadius() + 8.0f, this, @hitInfos ))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							//	map.server_DestroyTile(tpos, 0.01);
							//	ShakeScreen( 10, 6, tpos );
							}
						}
					}
				}		
...
//			this.getSprite().PlaySound("/BisonBoo");
	...
	if(getNet().isServer() && getGameTime() % 30 == 0)
	{
		if(this.get_u8(state_property) == MODE_TARGET )
		{
			CBlob@ b = getBlobByNetworkID(this.get_netid(target_property));
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
	...
	if (solid && target !is null)
	{
		Vec2f targetpos = target.getPosition();
		Vec2f thisPos = this.getPosition();
		Vec2f dir = Vec2f( XORRandom(16)-8, XORRandom(16)-8 )/8.0;
		dir.Normalize();
		Vec2f tilepos = thisPos + (dir)*(this.getRadius() + 4.0f);

		TileType tile = this.getMap().getTile( tilepos ).type;

		if ( targetpos.y <= thisPos.y && tilepos.y <=  thisPos.y)
		{		
			this.getMap().server_DestroyTile(tilepos, 20.8);
		}
		else if ( targetpos.y >= thisPos.y && tilepos.y >=  thisPos.y)
		{		
			this.getMap().server_DestroyTile(tilepos, 20.8);
		}
	}
...
void onHitBlob( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData )
{
	if (hitBlob !is null)
	{
		Vec2f force = velocity * this.getMass() * 5.35f ;
		force.y -= 1.0f;
