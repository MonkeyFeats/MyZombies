...
void onInit(CBlob@ this)
{
	...
	vars.walkForce.Set(difficulty*3.0f,0.0f);
...
void onTick( CMovement@ this )
{
	...
	if (left) {
		blob.AddForce(Vec2f( -1.0f * vars.walkForce.x, 0));
	}
	if (right) {
		blob.AddForce(Vec2f( 1.0f * vars.walkForce.x, 0));
	}
	...
	if (left || right || up)
	{
		...
		{ 
			f32 mod = blob.isInWater() ? 1.1f : 1.5f;
			blob.AddForce(Vec2f( mod*vars.jumpForce.x*blob.getMass(), mod*vars.jumpForce.y*blob.getMass()/2));
		...
		if (( (right && map.isTileSolid( Vec2f( pos.x + (radius+1.0f), pos.y ))) || (left && map.isTileSolid( Vec2f( pos.x - (radius+1.0f), pos.y )))))
		{
			...
			{
				f32 mod = blob.isInWater() ? 1.1f : 1.5f;
				blob.AddForce(Vec2f( mod*vars.jumpForce.x*blob.getMass()/1.9, mod*vars.jumpForce.y*blob.getMass()/2));
				climb++;
