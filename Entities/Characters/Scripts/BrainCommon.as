...
CBlob@ getNewTarget( CBrain@ this, CBlob @blob, const bool seeThroughWalls = true, const bool seeBehindBack = false )
{
	CBlob@[] players;
	...
	for (uint i=0; i < players.length; i++)
	{
		...
		if (potential !is blob && blob.getTeamNum() != potential.getTeamNum()
			&& (pos2 - pos).getLength() < 256.0f
			&& (isBot || seeBehindBack || Maths::Abs(pos.x-pos2.x) < 128.0f || (blob.isFacingLeft() && pos.x > pos2.x) ||  (!blob.isFacingLeft() && pos.x < pos2.x)) 
...
bool JustGo( CBlob@ blob, CBlob@ target )
{
	...
	//	if ((XORRandom(600) == 0) && point.y + getMap().tilesize*0.7f < mypos.y && (target.isOnGround() || target.getShape().isStatic())) {	  // dont hop with me
...
void JumpOverObstacles( CBlob@ blob )
{
	...
	if (blob.isOnWall() && (XORRandom(10) == 0)) {
		blob.setKeyPressed( key_up, true );
	}
	else
		if (!blob.isOnLadder())
...
//	if (targetPos.y + getMap().tilesize < mypos.y-16) {
//		blob.setKeyPressed( key_up, true );
//	}
}
